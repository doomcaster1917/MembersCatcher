...
from thread import Thread, Signals
import re
...
class GeneralView(QMainWindow):
    ...
    def open_process_widget_monitoring(self):
        ...
        self.ui.process_page_state_label.setText("Мониторится")
    ...
    def start_download_from_history(self):
        ...
        try:
            if not self.ui.process_page_chosen_chat.text():
                ...
                self.thread = Thread(chat_name=chat_name)
    ...
    def stop_button_handler(self):
        if self.ui.process_page_stop_button.text() == 'Остановить':
    ...
    def closeEvent(self, *args, **kwargs):
        process = self.signal_sender.process
        app = TelegramApi.app
        if app:
            app.stop()
        if process:
